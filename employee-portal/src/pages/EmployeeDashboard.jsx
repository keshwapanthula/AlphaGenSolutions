  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        ...
        <Link to="/timesheet/new" className="btn btn-primary">+ New Timesheet</Link>
      </div>
      ...
      {timesheets.length === 0 ? (
        <div className="portal-card empty-state">
          ...
          <Link to="/timesheet/new" className="btn btn-primary" style={{ marginTop: '1rem' }}>
            Create Timesheet
          </Link>
        </div>
      ) : (
            ...
            <tbody>
              {timesheets.map((ts) => (
                <tr key={ts._id}>
                  ...
                  <td>
                    <Link to={`/timesheet/${ts._id}`} className="btn btn-ghost btn-sm">
